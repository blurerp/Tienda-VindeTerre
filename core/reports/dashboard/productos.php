...
$pdf->startReport('Stock Activo de productos existentes por categoria');
    ...
    foreach ($dataCategorias as $rowCategoria) {
        ...
        if ($producto->setCategoria($rowCategoria['id_categoria'])) {
            ...
            if ($dataProductos = $producto->readProductosCategoria()) {
                ...
                $pdf->Cell(30, 10, utf8_decode('Stock Activo'), 1, 0, 'C', 1);
                $pdf->Cell(30, 10, utf8_decode('Estado'), 1, 0, 'C', 1);
                $pdf->Cell(30, 10, utf8_decode('Imagen'), 1, 0, 'C', 1);
                $pdf->Cell(36, 10, utf8_decode('Precio de venta'), 1, 1, 'C', 1);                
                ...
                foreach ($dataProductos as $rowProducto) {
                    ...
                    $pdf->Cell(30, 20, utf8_decode($rowProducto['stock_activo']), 1, 0);
                    $pdf->Cell(30, 20, utf8_decode($rowProducto['estado_producto']), 1, 0);
                    $pdf->Cell(30, 20,$pdf->Image('../../../resources/img/productos/'.$rowProducto['imagen_producto'] , $pdf->GetX(), $pdf->GetY(),20,20) ,1,0);
                    $pdf->Cell(36, 20, utf8_decode('$'.$rowProducto['precio_venta']), 1, 1);
